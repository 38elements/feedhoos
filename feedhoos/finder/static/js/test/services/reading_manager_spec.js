describe("readingManager", function() {
    ...
    it('should sortByRating sort data by rating of bookmarkManager by join id ',inject(function(readingManager, bookmarkManager) {
        $httpBackend.expect("GET", "/bookmark/list/")
            .respond('{' +
                '"0": {"rating": 6, "folder_id": 0},' +
                '"1": {"rating": 3, "folder_id": 1},' +
        ...
        bookmarkManager.set(scope, function() {});
        $httpBackend.flush();
        var data = readingManager.sortByRating(readingManager.data);
        expecting_data = [ 
            { 
                "id": 6, 
                "link": "http://example.com/entry/", 
